...
| Authenticated standalone runtime | `api/standalone-pilot-run.ts`; `api/standalone-pilot-run.test.ts`; `docs/standalone-pilot-runtime.md` | Default-off deployable entrypoint derives tenant/session/protocol/experiment/timestamps server-side; rejects legacy/local and cross-business sessions; supports shadow or receipt-backed sandbox assistance only | Deployment, bank source authentication, bank workflow receipt, production activation, or lift |
...
| Growth Play onboarding | `npm run test:growth-plays`; `npm run test:connector-sessions`; `backend/shared/growth-play-registry.test.mjs`; `docs/growth-play-control-plane.md` | Consumer and Merrill compile through one contract; a tenant-isolated append-only registry and distinct control-plane session enforce tenant, role, business-line, IdP lineage, and four-eyes approval before any evidence or model invocation | Actual bank approval, sanctioned mapping verification, deployed registry, or bank SSO/group mapping |
