...
| Growth Play onboarding | `npm run test:growth-plays`; `backend/shared/growth-play-contract.test.mjs`; `backend/shared/growth-play-registry.test.mjs`; `docs/growth-play-onboarding-contract.md` | Consumer and Merrill compile through one institution-neutral contract; a tenant-isolated append-only registry records approval/revocation and the loop fails before evidence or model invocation unless the exact protocol is approved | Actual bank approval, sanctioned mapping verification, deployed registry, or bank-identity-bound approver RBAC |
