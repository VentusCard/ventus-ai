...
| Authentication and authorization | Connector bearer compatibility gate; separate short-lived control-plane sessions with tenant, role, business-line, subject, IdP, and four-eyes enforcement; internal UI presentation gate; transaction-scoped tenant context and forced-RLS migration | Connector and control-plane RBAC contracts implemented and tested, not connected to bank SSO or deployed | Choose SSO/OIDC provider, map authoritative groups to Ventus roles/business lines, bind claims to tenant context, and test login/deprovisioning/isolation |
