...
| Institution-agnostic core with bank configuration | Compiled Growth Play protocol binds institution-specific source mappings, eligibility, policy, closed actions, routes, and outcomes to the provider-neutral loop; outcome IDs and event types are no longer hard-coded to the prototype plays | Implemented and tested for separate Consumer and Merrill drafts; not deployed | Bank-authored mapping approval, configuration registry, second-institution exercise, and successful isolation probe under the non-production runtime role |
