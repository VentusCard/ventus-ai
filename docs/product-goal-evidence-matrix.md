...
| --- | --- | --- | --- |
| Raw data → enrichment → financial state → recommendation → workflow → outcome | Leadership flow; Plaid-schema Live Lab; provider-neutral operating loop; default-off authenticated standalone runtime; source, decision, delivery, and outcome receipts; Measure step | Executable and demonstrated with fixtures; deployable runtime contract and Plaid source are unit-proven, not deployed | One sanctioned bank dataset, authenticated bank workflow receipt, and completed outcome window |
