...
| Governed connected expansion | Consumer-to-Merrill Skill remains separate from both standalone plays; outcome contract and operating loop can measure it independently | Implemented as an expansion contract, not required for MVP entry | Data authorization, entitlement design, and a pre-registered standalone-versus-connected lift comparison |
