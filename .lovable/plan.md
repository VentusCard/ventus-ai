...
## Goal
Turn the current hard swap into one staged visual handoff: the live Hawaii assistant from 8.3 moves left first, then the other phones fade in and the continuous carousel begins rolling.
...
- On the forward arrow from 8.3 to 8.4:
  ...
  2. Move the Hawaii phone smoothly from its centered 8.3 position to the left side, keeping its current size and conversation visible.
  3. Fade in the 8.4 heading, subtitle, edge masks, and the remaining phones around it.
  4. Once the fade-in settles, merge the Hawaii phone into the track and start the continuous right-to-left roll without a position or speed jump.
