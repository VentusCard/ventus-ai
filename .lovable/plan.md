...
Every row in the Recent transactions list on the Immediate Value slide is exactly one line — including the "JFK Vending Machine?" confirmation row. The amber "Confirm" / green "Confirmed" chip and its second line disappear from the collapsed row; confirming still happens inside the expanded detail.
...
1. **Collapsed row — single line for every transaction**
   ...
   - Keep the trailing "?" on the name while unconfirmed (`JFK Vending Machine?`) — it is the visual cue that this row needs confirmation, with no label. After the customer confirms inside the detail, the name settles to "JFK Vending Machine" (existing `confirmed` logic already does this).
