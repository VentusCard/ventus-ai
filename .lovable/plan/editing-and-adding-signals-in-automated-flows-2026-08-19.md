...
- New `src/components/tepilot/common/SaveSequence.tsx`: presentational spinner + stage text + check, with `inline` (button-sized) and `block` (row/card banner) variants. Uses existing semantic tokens and the light theme only — no dark utilities.
- Stage presets exported from the component file (`SIGNAL_STAGES`, `PLAYBOOK_STAGES`, `CONTENT_STAGES`) so surfaces stay consistent.
- Each save site replaces its direct `toast.success(...)` call with `run()`; the toast is dropped where the inline sequence already communicates success, and kept only for saves that happen inside a dialog that closes immediately.
- Timings: ~700ms per stage, ~900ms success hold, capped near 2.5s total. Respects `prefers-reduced-motion` by collapsing to a single "Saved" state.
