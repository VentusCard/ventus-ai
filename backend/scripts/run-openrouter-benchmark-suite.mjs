...
for (const model of models) {
  ...
  if (force || !existsSync(predictionsPath) || !existsSync(rawOutputPath)) {
    runNode('./scripts/capture-openrouter-benchmark-predictions.mjs', {
  ...
  if (force || !existsSync(reportPath)) {
    runNode('./scripts/evaluate-plaid-benchmark-run.mjs', {
      OPENROUTER_BENCHMARK_PREDICTIONS_PATH: predictionsPath,
...
runNode('./scripts/compare-plaid-benchmark-runs.mjs');
