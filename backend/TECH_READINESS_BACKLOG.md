...
- Separate DB credentials and model-provider credentials into separate Secrets Manager secrets. Runtime support is merged, `ventus/model-providers/gemini` exists, Lambda env vars/IAM are wired, the recovered backend package that reads `MODEL_PROVIDER_SECRET_ID` was deployed to the seven backend Lambdas on 2026-05-20, and the live cutover audit passes. Remaining cleanup: run authenticated enrichment smoke tests, then remove `GEMINI_API_KEY` from the DB credential secret.
