...
- Separate DB credentials and model-provider credentials into separate Secrets Manager secrets. Runtime support is merged, `ventus/model-providers/gemini` exists, Lambda env vars/IAM are wired, and the live cutover audit passes. Remaining cleanup: deploy the recovered backend package that reads `MODEL_PROVIDER_SECRET_ID`, smoke-test enrichment, then remove `GEMINI_API_KEY` from the DB credential secret.
