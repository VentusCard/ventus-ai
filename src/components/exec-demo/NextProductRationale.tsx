...
function CreditworthinessBanner({ assessment, loading }: { assessment?: CreditAssessment | null; loading: boolean }) {
  if (loading && !assessment) {
    return (
      <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 flex items-center gap-4">
        <div className="flex flex-col gap-1.5 shrink-0" style={{ width: 220 }}>
  ...
  const positives = (assessment.signals?.positive_indicators || []).slice(0, 2);
  const distress = (assessment.signals?.distress_indicators || []).slice(0, 2);

  const dirArrow = (d: "positive" | "negative" | "neutral") =>
    d === "positive" ? "↑" : d === "negative" ? "↓" : "→";
  const dirClass = (d: "positive" | "negative" | "neutral") =>
    d === "positive" ? "text-emerald-600"
    : d === "negative" ? "text-rose-600"
    : "text-slate-400";

  const Stat = ({ label, value }: { label: string; value: string }) => (
    <div className="flex flex-col gap-0.5 px-3">
      <div className="text-[8.5px] font-bold uppercase tracking-wider text-slate-400">{label}</div>
      <div className="text-[12px] font-bold text-slate-800 tabular-nums">{value}</div>
    </div>
  );
  ...
  return (
    <div
      ...
      style={{
        ...
        animation: `exec-product-reveal 0.4s ease-out both`,
      }}
    >
      <div className="px-4 py-3 flex items-stretch gap-4">
        {/* Left: label + band pill + summary + drivers */}
        <div className="flex flex-col gap-1.5 shrink-0" style={{ width: 260 }}>
          <div className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Creditworthiness</div>
          <div
          ...
          <p className="text-[11px] text-slate-600 leading-snug line-clamp-2">{assessment.summary}</p>
          {topDrivers.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-0.5">
              {topDrivers.map((dr, i) => (
                <span
                  key={i}
                  className="inline-flex items-center gap-1 text-[10px] font-medium rounded-full px-2 py-0.5 border border-slate-200 bg-slate-50 text-slate-700"
                  title={dr.explanation}
                >
                  <span className={`font-bold ${dirClass(dr.direction)}`}>{dirArrow(dr.direction)}</span>
                  {dr.label}
                </span>
              ))}
            </div>
        ...
        {/* Middle: affordability stats */}
        <div className="flex items-center gap-0 shrink-0">
          <Stat label="Monthly Inflow" value={formatSpend(aff?.estimated_monthly_inflow || 0)} />
          <div className="w-px h-8 bg-slate-200" />
          <Stat label="Monthly Outflow" value={formatSpend(aff?.estimated_monthly_outflow || 0)} />
          <div className="w-px h-8 bg-slate-200" />
          <Stat label="Surplus" value={`${Math.round((aff?.surplus_ratio || 0) * 100)}%`} />
          <div className="w-px h-8 bg-slate-200" />
          <Stat label="DTI Proxy" value={`${Math.round((aff?.estimated_dti_proxy || 0) * 100)}%`} />
        </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center gap-1">
          <div className="text-[8.5px] font-bold uppercase tracking-wider text-slate-400">Signals</div>
        ...
        {/* Right: suggested next step */}
        {topProduct && (
          <>
            <div className="w-px bg-slate-200 self-stretch shrink-0" />
            <div
              className="shrink-0 rounded-lg px-3 py-2 self-center"
              style={{ background: `${band.dot}10`, border: `1px solid ${band.dot}25`, width: 240 }}
            >
              ...
              <div className="text-[10.5px] text-slate-500 leading-snug mt-0.5 line-clamp-2">{topProduct.rationale}</div>
            </div>
          </>
        )}
      </div>
      <div className="px-4 pb-2 text-[9px] text-slate-400 italic text-right">
