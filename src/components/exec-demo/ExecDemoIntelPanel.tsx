        ...
        {chips.length > 0 && (
          <div>
            ...
            {synthesisTriggered && !activeTab ? (
              ...
              activeTab ? (
                /* ─── Compact rollup pills strip (tab is active) ─── */
                <div className="py-1">
                  <div className="flex flex-wrap gap-1.5">
                    {rollupStats.map((r, i) => (
                      <PillarRollupChip key={`${r.pillar}::${r.label}`} rollup={r} delay={0} isActive={activeRollup?.pillar === r.pillar && activeRollup?.label === r.label} onClick={() => onRollupClick?.(r)} />
                    ))}
                  </div>
                </div>
              ) : (
                /* ─── Full detail view with life events + risk (no tab selected) ─── */
