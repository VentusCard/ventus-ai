...
const EXTERNAL_SOURCES: SourceInput[] = [
  { label: "KYC & Profile", provider: "FIS · Core Banking", icon: UserCircle },
  { label: "ACH & Wires", provider: "FIS · Core Banking", icon: ArrowLeftRight },
  { label: "Deposits & Statements", provider: "FIS · Core Banking", icon: Database },
  { label: "Card Transactions", provider: "Fiserv · Card Processor", icon: CreditCard },
  { label: "Zelle", provider: "Early Warning Services", icon: Send },
  { label: "Digital Telemetry", provider: "App & web channels", icon: Smartphone },
  { label: "Credit File", provider: "Experian / TransUnion", icon: Gauge },
  { label: "Wealth Data", provider: "Credit Bureau", icon: PiggyBank, nonFcra: true },
  { label: "Property Data", provider: "Credit Bureau", icon: Home, nonFcra: true },
  { label: "Demographics Data", provider: "Credit Bureau", icon: Users, nonFcra: true },
];
...
function SourceRow({ input }: { input: SourceInput }) {
  const Icon = input.icon;
  return (
    <div className="relative flex items-center gap-2.5 px-3 py-2 rounded-lg border border-slate-200 bg-white shadow-sm hover:border-emerald-300 transition-colors">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <span className="text-[12.5px] font-semibold text-slate-900 leading-tight truncate">
            {input.label}
          </span>
          {input.nonFcra && (
...
function BankProductCard({
  ...
  return (
    <div
      className="relative rounded-lg border-2 border-dashed border-indigo-300 bg-gradient-to-br from-indigo-50/70 to-violet-50/70 shadow-sm hover:border-indigo-400 transition-colors"
    >
      <button
        type="button"
        onClick={onToggle}
      ...
      >
        <div className="flex items-center justify-center w-7 h-7 rounded-md shrink-0 border bg-indigo-600 text-white border-indigo-700">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[8.5px] font-bold uppercase tracking-wider px-1 py-0.5 rounded bg-indigo-600 text-white shrink-0">
              Internal
            </span>
          </div>
          <div className="text-[10.5px] text-indigo-700/80 leading-tight truncate mt-0.5">
            Single source of truth · {BANK_PRODUCT_TOTAL} products
          </div>
        </div>
        <ChevronDown
          className={cn(
            "w-3.5 h-3.5 text-indigo-400 shrink-0 transition-transform",
            isOpen && "rotate-180",
          )}
        />
        <span className="absolute top-1.5 right-2 w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse" />
      </button>
      {isOpen && (
        <div className="border-t border-indigo-200/60 px-3 py-2 space-y-1.5 animate-in fade-in slide-in-from-top-1 duration-150">
          {BANK_PRODUCT_CATEGORIES.map((c) => (
          ...
          {onOpenProducts && (
            <button
              ...
              className="mt-1 w-full flex items-center justify-between gap-1 text-[10.5px] font-semibold text-indigo-700 hover:text-indigo-800 hover:bg-indigo-100/60 px-1.5 py-1 rounded transition-colors"
            >
              <span>Open Products tab · {BANK_PRODUCT_TOTAL} products</span>
...
export function CapabilitiesView({ onOpenProducts }: { onOpenProducts?: () => void } = {}) {
  ...
  const [bankProductOpen, setBankProductOpen] = useState(false);
  const totalSourceInputs = EXTERNAL_SOURCES.length;
  const totalLeftRows = totalSourceInputs + 1; // + Bank Product
  ...
  return (
    <div className="space-y-6">
      <TabHeader
        ...
        howItWorks="Ventus wires into the cores, processors, and digital channels your bank already runs. Transactions, KYC, telemetry, and bureau data stream into the Behavioral Intelligence Core, get classified into five signal families, then fan back out to the CRM, rewards provider, digital banking app, marketing automation, advisor consoles, and risk ops."
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 lg:p-8">
        ...
        <div className="grid grid-cols-[260px_1fr_260px] gap-8 mb-4">
          <div className="flex items-center gap-1.5">
            ...
            <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-700">
              Sources · 1 internal catalog + {totalSourceInputs} external feeds
            </p>
          </div>
        ...
        <div className="relative">
          <NetworkWires leftCount={totalLeftRows} rightCount={DESTINATIONS.length} />

          <div className="relative z-10 grid grid-cols-[260px_1fr_260px] gap-8 items-stretch">
            {/* Sources */}
            <div className="flex flex-col gap-3">
              {/* Internal catalog — visually differentiated */}
              <div>
                <p className="text-[9px] font-bold uppercase tracking-[0.14em] text-indigo-700/80 mb-1.5 pl-0.5">
                  Internal catalog
                </p>
                <BankProductCard
                  isOpen={bankProductOpen}
                  onToggle={() => setBankProductOpen((v) => !v)}
                  onOpenProducts={onOpenProducts}
                />
              </div>

              {/* External feeds — individual items */}
              <div>
                <p className="text-[9px] font-bold uppercase tracking-[0.14em] text-slate-500 mb-1.5 pl-0.5">
                  External data feeds
                </p>
                <div className="flex flex-col gap-1.5">
                  {EXTERNAL_SOURCES.map((s) => (
                    <SourceRow key={s.label} input={s} />
                  ))}
                </div>
              </div>
            </div>
