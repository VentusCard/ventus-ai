...
const LedgerBand = () => {
  ...
  return (
    <section ref={sectionRef} className="w-full relative z-10" style={{ backgroundColor: DARK, paddingTop: 88, paddingBottom: 88 }}>
      <div className="max-w-7xl mx-auto px-6 md:px-8 grid gap-12 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] lg:items-center">
        <div>
          <p className="text-xs font-semibold tracking-widest uppercase mb-3" style={{ color: "#7EB3FF" }}>
            Decision Ledger
          </p>
          <h2 className="text-3xl md:text-[40px] font-bold text-white leading-tight mb-4">
            Every decision leaves a receipt.
          </h2>
          <p className="text-base leading-relaxed mb-6" style={{ color: "#8DA2C0" }}>
          ...
          <Link
            to="/demo/enterprise"
            className="inline-flex items-center gap-2 rounded-lg bg-white px-5 py-3 text-sm font-bold transition hover:bg-blue-50"
            style={{ color: DARK }}
          >
        ...
        <div className="rounded-2xl border p-5" style={{ borderColor: "#13244A", backgroundColor: "rgba(255,255,255,0.02)" }}>
          <div className="flex items-center justify-between gap-2 border-b pb-3" style={{ borderColor: "#13244A" }}>
            <span className="font-mono text-[11px] uppercase tracking-[0.2em]" style={{ color: "#5C7398" }}>
          ...
          <div className="mt-3 space-y-2 min-h-[280px]">
            {events.map((event) => (
              <div
                ...
                style={{ borderColor: "#13244A", backgroundColor: "rgba(255,255,255,0.03)", animation: "ventus-append 0.35s ease backwards" }}
              >
                <span className="w-9 flex-none font-mono text-[10px]" style={{ color: "#5C7398" }}>
                  #{String(event.seq).padStart(3, "0")}
                </span>
                <span className="w-16 flex-none font-mono text-[9px] font-bold tracking-wider" style={{ color: event.kind === "outcome" ? "#34D399" : "#7EB3FF" }}>
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block truncate text-[10px]" style={{ color: "#5C7398" }}>{event.detail}</span>
                </span>
                ...
                <span className="hidden sm:block flex-none font-mono text-[10px]" style={{ color: "#3D5578" }}>
                  {event.hash.slice(0, 6)}
                </span>
              </div>
            ))}
          </div>
          <p className={`mt-3 font-mono text-[10px] transition-opacity duration-700 ${complete ? "opacity-100" : "opacity-0"}`} style={{ color: "#5C7398" }}>
