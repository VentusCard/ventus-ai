      ...
      {delivered ? (
        <div className="p-5">
          ...
          <div className="mt-4 grid gap-px overflow-hidden rounded-md border sm:grid-cols-3" style={{ borderColor: "var(--v2-rule)", backgroundColor: "var(--v2-rule)" }}>
            {([["Decision receipt", moment.receipt?.records?.decision], ["Qualified referral", moment.receipt?.records?.referral], ["Employee task", moment.receipt?.records?.task]] as Array<[string, unknown]>).map(([label, record]) => <div key={label} className="bg-white p-3"><p className="v2-mono text-[9px] uppercase tracking-[0.1em]" style={{ color: "var(--v2-ink-faint)" }}>{label}</p>{record && typeof record === "object" && "url" in record && typeof record.url === "string" ? <a href={record.url} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-[12px] font-bold" style={{ color: "var(--c-accent)" }}>Open record <ArrowUpRight className="h-3.5 w-3.5" /></a> : <p className="mt-2 text-[12px] font-semibold" style={{ color: "var(--v2-ink-soft)" }}>Receipt pending reconciliation</p>}</div>)}
          </div>
