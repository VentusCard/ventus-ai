...
export default function MomentsPage() {
  ...
  return <div className="mx-auto grid max-w-6xl gap-8 xl:grid-cols-[minmax(0,0.34fr)_minmax(0,0.66fr)]">
    ...
    {selected && decision && action ? <MomentCard moment={selected} decision={decision} action={action} onSyncOutcome={() => void syncOutcome(selected.id)} syncingOutcome={syncingOutcome === selected.id} outcomeSyncMessage={outcomeSyncMessage} onRetryDelivery={canRespond && selected.status === "delivery_failed" ? () => void retryDelivery(selected.id) : undefined} retrying={activating === selected.id}>{canRespond && selected.status === "queued" ? <div>
      ...
      <button onClick={() => void activate(selected.id, selectedActionId)} disabled={!selected.policy.allowed || activating === selected.id} className="console-btn w-full !py-4 !text-[15px]"><Send className="h-4 w-4" />{selectedActionId === actions[0]?.id ? "Accept and deliver" : "Deliver approved alternative"}</button>
      <div className="mt-3 flex gap-4"><button onClick={() => setMode(mode === "modify" ? "none" : "modify")} className="text-[12px] font-semibold" style={{ color: "var(--c-accent)" }}>Choose alternative</button><button onClick={() => setMode("defer")} className="text-[12px] font-semibold" style={{ color: "var(--v2-ink-soft)" }}>Defer</button><button onClick={() => setMode("decline")} className="text-[12px] font-semibold" style={{ color: "var(--v2-ink-soft)" }}>Decline</button></div>
