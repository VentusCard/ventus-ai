...
export default function MomentsPage() {
  ...
  return <div className="mx-auto grid max-w-6xl gap-8 xl:grid-cols-[minmax(0,0.34fr)_minmax(0,0.66fr)]">
    ...
    {selected && decision && action ? <MomentCard moment={selected} decision={decision} action={action} onSyncOutcome={() => void syncOutcome(selected.id)} syncingOutcome={syncingOutcome === selected.id} outcomeSyncMessage={outcomeSyncMessage}>{canRespond && selected.status === "queued" ? <div>
