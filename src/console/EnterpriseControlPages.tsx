...
import { Check, ChevronRight, CircleAlert, FileText as FileTextIcon, Loader2, Mail, Plug, Send, ShieldCheck, SlidersHorizontal } from "lucide-react";
...
export function ResultsPage() {
  ...
  const canSyncFscOutcome = ["bank_operator", "growth_play_owner", "risk_reviewer"].includes(access?.role ?? "");
  ...
  const syncSalesforceOutcome = async (decisionId: string) => {
    try {
      ...
      const result = await serverRequest<{ outcome: { outcome?: { status?: string } }; recorded?: { observation?: { observationId?: string }; measurement?: { status?: string; eventId?: string; reason?: string } } }>(session?.access_token, consoleSalesforceOutcomeSyncUrl(), { method: "POST", body: JSON.stringify({ decisionId }) });
      const measurement = result.recorded?.measurement;
      setError(measurement?.status === "recorded" ? `Salesforce outcome recorded as formal measurement ${measurement.eventId ?? "event"}. Evaluation remains gated by sample and holdout coverage.` : `Salesforce outcome ${result.outcome.outcome?.status ?? "awaiting outcome"} recorded as ${result.recorded?.observation?.observationId ?? "a durable observation"}. ${measurement?.reason ? measurement.reason.replaceAll("_", " ") : "Lift remains gated by the experiment design."}`);
      await load();
    } catch (cause) {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <div className="console-cell mt-5 overflow-hidden"><div className="grid grid-cols-[1.25fr_.65fr_.65fr_.65fr_.9fr] gap-3 border-b bg-[#f7f6f2] px-4 py-2" style={{ borderColor: "var(--v2-rule)" }}>{["Experiment", "Treatment", "Holdout", "Coverage", "Evaluation"].map((item) => <p key={item} className="v2-mono text-[8px] font-bold uppercase tracking-[.12em]" style={{ color: "var(--v2-ink-faint)" }}>{item}</p>)}</div>{data.experiments.length ? data.experiments.map((item) => {
      ...
      return <div key={item.experimentId} className="border-b px-4 py-3 last:border-0" style={{ borderColor: "var(--v2-rule)" }}><div className="grid grid-cols-[1.25fr_.65fr_.65fr_.65fr_.9fr] gap-3"><div><p className="text-[12px] font-bold">{item.experimentId}</p><p className="text-[10px]" style={{ color: "var(--v2-ink-faint)" }}>{item.evidenceClass} · {item.claimStatus.replaceAll("_", " ")}</p>{canExportEvidence && <button onClick={() => void exportEvidence(item.experimentId)} disabled={exporting === item.experimentId} className="mt-2 flex items-center gap-1 text-[10px] font-semibold" style={{ color: "var(--c-accent)" }}>{exporting === item.experimentId ? <Loader2 className="h-3 w-3 animate-spin" /> : <FileTextIcon className="h-3 w-3" />} Export evidence</button>}</div><p className="text-[12px]">{item.treatmentAssigned}</p><p className="text-[12px]">{item.holdoutAssigned}</p><p className="text-[12px]">{Math.round(item.coverage * 100)}%</p><div><p className="text-[11px] font-semibold">{item.sampleReady ? item.confidence.replaceAll("_", " ") : "sample building"}</p><p className="text-[10px]" style={{ color: "var(--v2-ink-faint)" }}>{item.intentToTreatLift === null ? "ITT not ready" : `ITT ${item.intentToTreatLift.toFixed(2)}`}</p></div></div>{protection && <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 border-t pt-3 text-[10px]" style={{ borderColor: "var(--v2-rule)", color: "var(--v2-ink-soft)" }}><span className="font-semibold" style={{ color: protection.status === "verified" ? "var(--v2-verified)" : "#b3261e" }}>{protection.status === "verified" ? "Holdout protected" : protection.status === "unavailable" ? "Holdout receipt unavailable" : "Holdout review required"}</span><span>{protection.assigned} reserved</span><span>{protection.decisionEvents} Moments</span><span>{protection.activationEvents} activations</span><span>{protection.workflowRecords.count} workflow records</span></div>}</div>;
...
function SkillShadowPanel({ accessToken, canManage }: { accessToken: string | undefined; canManage: boolean }) {
  ...
  useEffect(() => { void load().catch((cause) => setMessage(cause instanceof Error ? cause.message : "Skill registry is unavailable")); }, [accessToken]);
  const save = async () => {
    try {
      setBusy(true);
      setMessage(null);
