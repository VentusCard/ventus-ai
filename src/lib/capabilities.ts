...
export const CAPABILITIES: readonly CapabilityDefinition[] = [
  ...
  },
  {
    ...
    internalReality: "A real Salesforce Task write was operator-confirmed in a test org. System-native payloads, short-lived tenant/scope/destination sessions, and an at-most-once receipt contract are implemented; the connector remains sandbox-only, while bank identity and durable receipt-store verification remain open.",
    status: "pilot-scope",
    leadershipVisible: true,
  ...
  },
  {
    ...
    internalReality: "SHA-256 append, idempotency, tenant serialization, integrity export, forced-RLS migrations, and live verification scripts are implemented and tested; Evidence Store infrastructure and a generated non-bypass runtime credential are deployed in non-production, while migration, runtime-role isolation, and DATABASE_URL cutover verification remain pending.",
    status: "evaluation-locked",
    leadershipVisible: false,
    evidenceGate: ["Verify append-only store", "Verify RLS and restore integrity", "Approve tenant retention policy"],
  },
  {
    id: "multi-model",
