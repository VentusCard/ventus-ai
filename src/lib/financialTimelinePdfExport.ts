...
/** Return the same PDF as a base64 string (without the data-uri prefix). */
export async function generateFinancialTimelinePDFBase64(projection: SavedFinancialProjection): Promise<string> {
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.getWidth();
  let yPos = 20;

  // --- identical generation logic (duplicated to keep the download helper untouched) ---
  ...
  const totalCost = projection.costCategories.reduce((sum, cat) => sum + Object.values(cat.amounts).reduce((s, v) => s + (v || 0), 0), 0);
  const totalFunding = projection.fundingSources.reduce((sum, source) => sum + Object.values(source.amounts).reduce((s, v) => s + (v || 0), 0), 0);
  const fundingGap = totalCost - totalFunding;

  doc.text(`Total Projected Cost: ${formatCurrency(totalCost)}`, 25, yPos); yPos += 6;
  doc.text(`Total Funding: ${formatCurrency(totalFunding)}`, 25, yPos); yPos += 6;
  doc.setFont("helvetica", "bold");
  doc.text(`Funding Gap: ${formatCurrency(fundingGap)}`, 25, yPos); yPos += 10;
  ...
  // Return base64 string (strip the "data:application/pdf;base64," prefix)
