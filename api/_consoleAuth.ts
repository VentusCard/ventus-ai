...
export async function authorizeConsoleUser(request: Request): Promise<ConsolePrincipal | null> {
  ...
  if (!operatorAllowed(email)) return null;
  ...
  return {
    userId,
    email,
    tenantId: configuredTenant || tenantForEmail(email),
    role,
  };
}

function operatorAllowed(email: string): boolean {
  const exactEmails = csv("VENTUS_CONSOLE_ALLOWED_EMAILS");
  const allowedDomains = csv("VENTUS_CONSOLE_ALLOWED_DOMAINS", ["ventusai.com"]);
