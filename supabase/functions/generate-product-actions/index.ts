...
serve(async (req) => {
  ...
  try {
    ...
    const bankPrefix = bankName
      ? `BANK IDENTITY: You represent ${bankName}${bankShort ? ` ("${bankShort}")` : ""}. Where action labels reference the bank, use "${bankShort || bankName}" instead of "Our Bank" or "the bank".${bankWebsite ? ` Reference site: ${bankWebsite}.` : ""}\n\n`
      : "";
