...
/**
 * Retirement shell for the Evidence Store migrator. The retained runtime
 * secret and migrator log group keep their identities for reviewed rollback;
 * no migration Lambda or database-accessing policy remains.
 */
export class VentusEvidenceStoreStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    super(scope, id, props);
    const databaseSecretsKey = kms.Alias.fromAliasName(
      this, 'ExistingDatabaseSecretsKey', 'alias/ventus/database-secrets',
    );
    const runtimeSecret = new secretsmanager.Secret(this, 'VentusEvidenceAppV1Secret', {
      ...
      generateSecretString: {
        ...
        generateStringKey: 'password', passwordLength: 40, excludePunctuation: true,
      },
    ...
    new cdk.CfnOutput(this, 'EvidenceStoreRuntimeSecretName', { value: runtimeSecret.secretName });
    new cdk.CfnOutput(this, 'EvidenceStoreSchemaName', { value: 'ventus_evidence' });
  }
}
